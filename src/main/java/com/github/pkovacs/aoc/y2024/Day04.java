...
public class Day04 extends AbstractDay {
    ...
    private static long countXmasRays(CharTable table, Pos p) {
        if (table.get(p) != 'X') {
            return 0;
        }

        return table.neighbors8(p)
                .map(n -> table.ray(p, n).map(table::get).limit(3).toList())
                .filter(List.of('M', 'A', 'S')::equals)
                .count();
    }

    private static boolean isXmasCenter(CharTable table, Pos p) {
        if (table.get(p) != 'A') {
            return false;
        }

        var diagonalNeighbors = table.neighbors8(p).filter(n -> n.x != p.x && n.y != p.y).toList();
        return diagonalNeighbors.size() == 4 && diagonalNeighbors.stream().allMatch(n ->
                (table.get(n) == 'M' || table.get(n) == 'S') && table.get(n.mirrorAcross(p)) != table.get(n));
    }
